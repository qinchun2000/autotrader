...
/// POST请求
string HttpBoost::PostRequest(char* host, char* path, string form)
{
...
/// GET请求
string HttpBoost::GetRequestHost(char* host, char* path)
{
    // 声明Asio基础: io_service（任务调度机）
    boost::asio::io_service io_service;
    ...
    while (getline(response_stream, header) && header != "\r")
    {
        if (header.find("Content-Length: ") == 0) {
            ...
            stream << header.substr(16);
            stream >> len;
            printf("header.substr(16)   ---->%s\n",header.substr(16).c_str());
            printf("len   ---->%d \n",len);

        }

        printf("  while    ---->header: %s \n",header.c_str());
    }
...
/// GET请求(重载)
string HttpBoost::GetRequest(string url)
{
    size_t index;
    ...
    return GetRequestHost(host, urlPath);
}
