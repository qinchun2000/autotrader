...
Mdzmq::Mdzmq(DataWrapper * datawrapper,MarketBase *marketbase,const char* staname){
...
void Mdzmq::SetStrategyName(const char* strategyname){
	this->m_strategyname=strategyname;

}

std::string Mdzmq::GetStrategyName(){
	return this->m_strategyname;
}

std::string Mdzmq::RouterPort(){
	std::string port;
	port="5888";
...
//	    	std::cout<<"zmq>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>   port:"<<address<<std::endl;
	        ...
	        case 201:
	        	{
				  std::string str_ins=this->ProcessSubMarket(req.c_str());
...
int Mdzmq::GetReqCode(const char* req)
{
...
std::string Mdzmq::ProcessSubMarket(const char* req)
{
	std::string str_req=req;
	std::string userid;
	std::string ret;
...
std::string Mdzmq::PackHoldPosition(const char* userid){
...
std::string Mdzmq::PackError(int errorcode){
