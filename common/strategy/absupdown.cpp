...
#include "absupdown.hpp"

AbsUpDown::AbsUpDown() {

//	printf("AbsUpDown  无参数构造函数-->初始  \n");
}


AbsUpDown::AbsUpDown(DataWrapper * datawrapper,const char* strategyname) {

	this->SetRedisDepthMarket(&m_redisdepthmarket);
	this->SetRedisSignal(&m_redissignal);
	this->SetRedisSection(&m_redissection);

	this->GetLogUtil()->WriteLog("AbsUpDown  带参数构造函数-->初始开始  \n");
...
//	printf("AbsUpDown  带参数构造函数-->初始结束   Open signalcount:%d \n",this->GetOpenSignalCount());

	this->GetLogUtil()->WriteLog("AbsUpDown  带参数构造函数-->初始结束 ");
}
void AbsUpDown::run(){
...
//	std::cout<<tradingday_sqltime<<endl;
	...
	std::string funname=">>>>>>>>>AbsUpDown::Open";
...
//					printf(">>>>>>>>>	请检查白盘合约在夜盘无数据？: %s    \n",item->InstrumentID);
				...
				printf(">>>>>>>>>   AbsUpdown策略验证 --> [%s]不存在Redis行情数据 !!!  请检查行情是否接收正常？   \n",item->InstrumentID);
			}
...
void  AbsUpDown::Close()
{
	DateUtil dateutil;
	int holdcount=this->GetDataWrapper()->GetHoldingPosition();
	if(holdcount>0)	{
		printf(">>>>>>>>>>>>>>>>>AbsUpDown::Close: 所有持仓品种----->当日持仓批次[%d] ----> %s\n",holdcount,
...
//		std::cout<<">>>>>>>AbsUpDown hold empty"<<endl;
	}
