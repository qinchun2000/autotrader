...
#include "bigvolume_sed.hpp"

BigVolume_Sed::BigVolume_Sed() {
	printf("BigVolume_Sed  无参数构造函数-->初始  \n");
}


BigVolume_Sed::BigVolume_Sed(DataWrapper * datawrapper,const char* strategyname) {

	this->SetRedisDepthMarket(&m_redisdepthmarket);
	this->SetRedisSignal(&m_redissignal);
	this->SetRedisSection(&m_redissection);

	printf("BigVolume_Sed  带参数构造函数-->初始开始  \n");
	...
	printf("BigVolume_Sed  带参数构造函数-->初始结束   Open signalcount:%d \n",this->GetOpenSignalCount());

	this->GetLogUtil()->WriteLog("BigVolume_Sed  带参数构造函数-->初始结束 ");
}
void BigVolume_Sed::run(){
...
//		gettimeofday( &m, NULL );
//		printf(">>>>>>>>>>>>>>>BigVolume_Sed::run() ----------------> %ld.%ld\n", m.tv_sec, m.tv_usec);
...
//	std::cout<<">>>>>>>>>>>>>>> tradingday_sqltime:"<<tradingday_sqltime<<endl;
	...
	std::string funname=">>>>>>>>>>>>>>>>BigVolume_Sed::Open";
...
//								depthmarket.LastPrice,m,s,stopprice,depthmarket.PriceTick);
			...
			if(position==0){
//				printf(">>>>>>>>>>>	BigVolume_Sed::Open--> 出现开仓信号 %s ,基于风控最终开仓量为0!!!----> %d   \n",
//												signalname.c_str(),position);
//				sprintf(logbuf,"BigVolume_Sed::Open--> 出现开仓信号 %s ,基于风控最终开仓量为0!!!----> %d   \n",
//						signalname.c_str(),position);
//				this->GetLogUtil()->WriteLog(logbuf);
			}
			else {

				printf(">>>>>>>>>>>	BigVolume_Sed::Open 开多 --> %s 现价:%.2f  --->%s 可开仓位: %d \n",
...
//				this->GetLogUtil()->WriteLog(logbuf);
				...
				printf(">>>>>>>>>>>BigVolume_Sed::Open 开空 --> %s 现价:%.2f    --->%s   可开仓位: %d \n",
							pinstrumentid,depthmarket.LastPrice,depthmarket.UpdateTime ,position);
			}
			Signal signal;
			strcpy(signal.InstrumentID,item->InstrumentID);
...
void  BigVolume_Sed::opentest()
{
//	printf("Strategy::Open_BigVolumOneMinSignal  begin----------->  \n");

	char logbuf[1024];
	double avalible = this->GetInitialMoney() ;
//	printf(">>>>>>>>>>>>>>>BigVolume_Sed::Open  begin----------->	 可用资金: %.2f  \n",avalible);
	 double bigvolumerate =this->GetBigVolumeRate();
...
//								depthmarket.LastPrice,m,s,stopprice,depthmarket.PriceTick);
			...
			int position =  this->GetPositionByAvalibleATR(avalible,pinstrumentid, depthmarket,atr20,commssion->Margin);
			if(position==0){
				printf(">>>>>>>>>>>BigVolume_Sed:opentest--> 出现开仓信号 %s ,基于风控最终开仓量为0!!!----> %d   \n",
												signalname.c_str(),position);
				sprintf(logbuf,">>>>>>>>>>>BigVolume_Sed:opentest--> 出现开仓信号 %s ,基于风控最终开仓量为0!!!----> %d   \n",
						signalname.c_str(),position);
				this->GetLogUtil()->WriteLog(logbuf);
			}
			else {

//				printf(">>>>>>>>>>>BigVolume_Sed:opentest 开多 --> %s 现价:%.2f  --->%s 可开仓位: %d \n",
//										pinstrumentid,depthmarket.LastPrice,depthmarket.UpdateTime ,position);
			}
			Signal signal;
			strcpy(signal.InstrumentID,item->InstrumentID);
...
//				printf(">>>>>>>>>>>BigVolume_Sed:opentest 开空 --> %s 现价:%.2f    --->%s   可开仓位: %d \n",
//							pinstrumentid,depthmarket.LastPrice,depthmarket.UpdateTime ,position);
			}
			Signal signal;
			strcpy(signal.InstrumentID,item->InstrumentID);
...
//	printf(">>>>>>>>>>>>>>>>>BigVolume_Sed   持仓明细表持仓数[%d] ----> %s\n",detailposition,
//		dateutil.ConvertSHFETimetoSqlTime(this->GetDifSec()).c_str());

	if(detailposition)	{
//		printf(">>>>>>>>>>>>>>>>>BigVolume_Sed::Close: 所有持仓品种----->当日持仓批次[%ld] ----> %s\n",list.size(),
//				dateutil.ConvertSHFETimetoSqlTime(this->GetDifSec()).c_str());
