...
void MysqlUser::UpdatebyStrategyName(User data) {

	char buf[2048];
	std::string str_sql;

	sprintf(buf,"UPDATE %s SET Password='%s',Actived='%d',UserID='%s' WHERE StrategyName='%s' ",
			this->getTablename().c_str(), data.Password, data.Actived, data.UserID,data.StrategyName);
